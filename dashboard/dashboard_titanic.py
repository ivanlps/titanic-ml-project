...
def _predict_proba_safe(model, X):
    """Devuelve la probabilidad de supervivencia de forma robusta.
    Si el modelo no tiene predict_proba, intenta mapear la salida a [0,1].
    """
...
def _apply_random_uplift_on_proba(p, low=0.1, high=0.3, n=200, seed=42):
    """Aplica un uplift aleatorio en espacio de log-odds sobre la probabilidad p.
    Retorna un array de probabilidades simuladas.
    """
    rng = np.random.default_rng(seed)
    p = float(np.clip(p, 1e-6, 1-1e-6))
    logit = np.log(p / (1 - p))
    uplifts = rng.uniform(low, high, size=n)
    logit_new = logit + uplifts
...
with st.sidebar:
    ...
    st.markdown("---")
    page = st.radio("Navegar a:", ["Exploración", "Predicción", "What-If"])
...
def page_what_if():
    ...
    # Modelos disponibles y orden deseado (XGBoost primero, Logistic al final)
    disponibles = [k for k, v in models_loaded.items() if v is not None]
    if not disponibles:
        st.info("No hay modelos cargados.")
        return
    orden = ["XGBoost/LightGBM", "Random Forest", "SVM", "Logistic Regression"]
    disponibles = [m for m in orden if m in disponibles]

    model_name = st.selectbox("Modelo", options=disponibles, key="whatif_model", index=0)
    model = models_loaded[model_name]
    ...
    try:
        ...
        proba_base = _predict_proba_safe(model, X_model)
        st.metric("Probabilidad base de supervivencia", f"{(proba_base*100):.2f}%")

        # ==============================
        # Análisis de sensibilidad (edad y tarifa)
        # ==============================
        ...
        if st.checkbox("Analizar impacto de la edad"):
            ...
            for test_age in ages:
                X_test = X_base.copy()
                X_test["Age"] = [float(test_age)]
                X_test_model = preprocess_for_model(model, X_test)
                probas.append(_predict_proba_safe(model, X_test_model) * 100)
            if px is not None:
        ...
        if st.checkbox("Analizar impacto de la tarifa"):
            ...
            for test_fare in fares:
                ...
                X_test_model = preprocess_for_model(model, X_test)
                probas_fare.append(_predict_proba_safe(model, X_test_model) * 100)
            if px is not None:
        ...
        # ==============================
        # Escenarios ilustrativos (no causal)
        # ==============================
        st.markdown("---")
        st.subheader("Escenarios ilustrativos (no causal)")
        tabs = st.tabs(["Caso A: Cambio de sexo + ruido", "Caso B: Más botes (uplift aleatorio)"])

        # -------- Caso A: cambio de sexo + ruido aleatorio --------
        with tabs[0]:
            st.caption("Toma el pasajero base, invierte 'Sex' y aplica un uplift aleatorio en log-odds sobre la probabilidad resultante. No es inferencia causal.")

            # Crear escenario con sexo invertido
            X_flip = X_base.copy()
            X_flip["Sex"] = ["female" if X_base["Sex"].iloc[0] == "male" else "male"]

            X_base_m = preprocess_for_model(model, X_base)
            X_flip_m = preprocess_for_model(model, X_flip)

            p_base = _predict_proba_safe(model, X_base_m)
            p_flip = _predict_proba_safe(model, X_flip_m)

            colA, colB, colC = st.columns(3)
            with colA:
                low = st.number_input("Uplift mínimo (log-odds)", value=0.10, min_value=0.00, max_value=2.00, step=0.05)
            with colB:
                high = st.number_input("Uplift máximo (log-odds)", value=0.30, min_value=0.00, max_value=3.00, step=0.05)
            with colC:
                n_sims = st.number_input("# simulaciones", value=200, min_value=10, max_value=5000, step=10)

            seed = st.number_input("Semilla", value=42, min_value=0, max_value=10000, step=1)
            ejecutar = st.button("Aplicar escenario (Cambio de sexo)")

            if ejecutar:
                if high < low:
                    st.warning("El máximo debe ser mayor o igual que el mínimo.")
                else:
                    sims = _apply_random_uplift_on_proba(p_flip, low=low, high=high, n=int(n_sims), seed=int(seed))
                    p_mean = float(np.mean(sims))

                    c1, c2, c3 = st.columns(3)
                    with c1:
                        st.metric("Prob. base (sexo original)", f"{p_base*100:.2f}%")
                    with c2:
                        st.metric("Prob. flip (sin ruido)", f"{p_flip*100:.2f}%")
                    with c3:
                        st.metric("Prob. escenario (media sims)", f"{p_mean*100:.2f}%")

                    if px is not None:
                        try:
                            fig = px.histogram(sims, nbins=20, title="Distribución de probabilidades simuladas")
                            fig.update_xaxes(title="Probabilidad simulada")
                            fig.update_yaxes(title="Frecuencia")
                            st.plotly_chart(fig, use_container_width=True)
                        except Exception as e:
                            st.warning(f"No se pudo graficar el histograma: {e}")

                    st.info("Escenario demostrativo con ruido aleatorio sobre log-odds. No representa causalidad.")

        # -------- Caso B: “más botes” como uplift aleatorio --------
        with tabs[1]:
            st.caption("Simula el efecto de 'más botes' como un uplift aleatorio aplicado a la probabilidad base. Ilustrativo, no causal.")
            ...
            colA, colB, colC = st.columns(3)
            with colA:
                low2 = st.number_input("Uplift mínimo (log-odds)", value=0.20, min_value=0.00, max_value=3.00, step=0.05, key="mb_low")
            with colB:
                high2 = st.number_input("Uplift máximo (log-odds)", value=0.60, min_value=0.00, max_value=4.00, step=0.05, key="mb_high")
            with colC:
                n_sims2 = st.number_input("# simulaciones", value=200, min_value=10, max_value=5000, step=10, key="mb_n")

            seed2 = st.number_input("Semilla", value=7, min_value=0, max_value=10000, step=1, key="mb_seed")
            ejecutar2 = st.button("Aplicar escenario (Más botes)")

            if ejecutar2:
                if high2 < low2:
                    st.warning("El máximo debe ser mayor o igual que el mínimo.")
                else:
                    sims2 = _apply_random_uplift_on_proba(p_base, low=low2, high=high2, n=int(n_sims2), seed=int(seed2))
                    p_mean2 = float(np.mean(sims2))

                    c1, c2 = st.columns(2)
                    with c1:
                        st.metric("Prob. base", f"{p_base*100:.2f}%")
                    with c2:
                        st.metric("Prob. escenario (media sims)", f"{p_mean2*100:.2f}%")

                    if px is not None:
                        try:
                            fig2 = px.histogram(sims2, nbins=20, title="Distribución simulada (Más botes)")
                            fig2.update_xaxes(title="Probabilidad simulada")
                            fig2.update_yaxes(title="Frecuencia")
                            st.plotly_chart(fig2, use_container_width=True)
                        except Exception as e:
                            st.warning(f"No se pudo graficar el histograma: {e}")

                    st.info("Escenario demostrativo con uplift aleatorio; no modela logística, tiempos ni asignación real de botes.")
    except Exception as e:
